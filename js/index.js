document.addEventListener('DOMContentLoaded', async event => {
    ...
    const testResetButton = document.getElementById('button-test-reset');
    ...
    const fileUploadTestReset = document.getElementById('file-upload-test-reset');
                ...
                switch (id) {
                    case IMG_MGMT_ID_STATE:
                        ...
                        if (images) {
                            ...
                            testResetButton.disabled = !(data.images.length > 1 && data.images[1].pending === false);
                            confirmButton.disabled = !(data.images.length > 0 && data.images[0].confirmed === false);
                        } else {
                            testButton.disabled = true;
                            testResetButton.disabled = true;
    ...
    mcumgr.onImageUploadFlash(async () => {
        fileStatus.innerText = 'Upload complete';
        fileInfo.innerHTML = '';
        fileImage.value = '';
    ...
    fileImage.addEventListener('change', () => {
        ...
        reader.onload = async () => {
            ...
            try {
                ...
                fileUploadTestReset.disabled = false;
            } catch (e) {
    ...
    fileUpload.addEventListener('click', event => {
        ...
        fileUploadTestReset.disabled = true;
        event.stopPropagation();
        if (file && fileData) {
            mcumgr.cmdUpload(fileData);
        }
    });
    fileUploadTestReset.addEventListener('click', event => {
        fileUpload.disabled = true;
        fileUploadTestReset.disabled = true;
        event.stopPropagation();
        if (file && fileData) {
            mcumgr.cmdUploadAndProgram(fileData);
        }
    });
    ...
    testResetButton.addEventListener('click', async () => {
        if (images.length > 1 && images[1].pending === false) {
            await mcumgr.cmdImageTest(images[1].hash);
            await mcumgr.cmdReset();
        }
